...
const SSR_POOL: SsrCard[] = [
  { id: "defensive_ssr_chibi",   name: "Analyst · 치비",   image: "/card_ssr_chibi.png",   line1: "분석은 재미있어!",               line2: "세상을 이해하는 방법이거든!" },
  { id: "defensive_ssr_concert", name: "Analyst · 콘서트", image: "/card_ssr_concert.png", line1: "우리의 데이터는 반짝이는 가능성이야.", line2: "지금, 함께 성장하자!" },
  { id: "defensive_ssr_surfer",  name: "Analyst · 서퍼",   image: "/card_ssr_surfer.png",  line1: "끝없이 펼쳐진 수평선처럼,",          line2: "우리의 가능성도 무한해!" },
  { id: "defensive_ssr_magical", name: "Analyst · 매지컬", image: "/card_ssr_magical.png", line1: "데이터도, 미래도, 내 손안에 있어!",   line2: "함께라면 뭐든지 가능해!" },
  { id: "defensive_ssr_gamer",   name: "Analyst · 게이머", image: "/card_ssr_gamer.png",   line1: "게임은 전략이야.",                  line2: "승리도, 성장도 계획이 필요하지!" },
  { id: "defensive_ssr_mystic",  name: "Analyst · 미스틱", image: "/card_ssr_mystic.png",  line1: "진실은 언제나 데이터 속에 숨어 있어.", line2: "내가 찾아낼 뿐이지." },
];
...
function collectedKey(): string { return `wonkwang_gacha_collected_${getUid()}`; }
function historyKey(): string   { return `wonkwang_gacha_history_${getUid()}`; }

function loadCollected(): string[] {
  try { return JSON.parse(localStorage.getItem(collectedKey()) ?? "[]"); } catch { return []; }
}
function saveCollected(items: string[]): void {
...
function generatePull(): PullResult[] {
  return Array.from({ length: PULL_COUNT }, (_, i) => {
    ...
    return { idx: i, type: "miss" as const, card: null };
  });
}
...
export default function GachaShop({ userId: _userId }: GachaShopProps) {
  ...
  const [savedPull, setSavedPull] = useState<boolean>(false);
  const [cinematic, setCinematic] = useState<SsrCard | null>(null);
  ...
  useEffect(() => {
    if (!results || !allRevealed || savedPull) return;
    setSavedPull(true);
    const ssrIds = results.filter((r) => r.type === "ssr" && r.card).map((r) => r.card!.id);
    const entry: HistoryEntry = { timestamp: Date.now(), ssrCards: ssrIds };
    const next = [entry, ...history];
    setHistory(next);
    saveHistory(next);
    const newCollected = [...collected];
    ssrIds.forEach((id) => { if (!newCollected.includes(id)) newCollected.push(id); });
    if (newCollected.length !== collected.length) {
      setCollected(newCollected);
      saveCollected(newCollected);
    }
  }, [allRevealed, results, savedPull, history, collected]);
  ...
  function handleCardClick(i: number) {
    ...
    } else if (results && results[i].type === "ssr" && results[i].card) {
      setCinematic(results[i].card);
    }
  }

  function handleRevealAll() {
    setRevealed(new Array(PULL_COUNT).fill(true));
  }
  ...
  const hasSsr = (results ?? []).some((r) => r.type === "ssr");
      ...
      <AnimatePresence>
        {showModal && results && (
          ...
          >
            ...
            >
              <h2 className="text-white font-extrabold text-xl text-center mb-1">🎰 10연차 결과</h2>

              {!allRevealed ? (
                <p className="text-white/40 text-[11px] text-center mb-4">
                  카드를 탭해서 하나씩 열거나 아래 버튼으로 한번에 열 수 있어요
                </p>
              ) : hasSsr ? (
                <p className="text-yellow-400 text-[11px] font-bold text-center mb-4">
                  ✨ 황금 카드를 탭하면 시네마틱이 열려요!
                </p>
              ) : (
                <p className="text-white/40 text-[11px] text-center mb-4">
                  다음엔 SSR이 뜰지도 몰라요!
                    ...
                    <AnimatePresence mode="wait">
                      {!revealed[i] ? (
                        <motion.div
                          ...
                          className="absolute inset-0 rounded-xl bg-gradient-to-br from-violet-800 to-purple-900 flex flex-col items-center justify-center gap-1 border border-violet-500/50 hover:border-violet-400 active:scale-95 transition-transform"
                        >
                          <span className="text-white/60 text-lg font-bold">?</span>
                          <span className="text-[7px] text-violet-300/60 font-bold tracking-widest">TAP</span>
                        </motion.div>
              ...
              <div className="space-y-2">
                {!allRevealed && (
                  <motion.button
                    ...
                    className="w-full bg-violet-500 hover:bg-violet-600 active:scale-[0.98] text-white font-extrabold py-3 rounded-2xl transition-all"
                  >
                    한번에 열기
                  </motion.button>
                )}
        ...
        {cinematic && (
          ...
          >
            ...
            {Array.from({ length: 22 }, (_, i) => {
              const size = 3 + (i * 7) % 5;
              const left = 15 + (i * 31) % 70;
              const top  = 10 + (i * 43) % 80;
              const delay = 0.2 + (i * 0.037);
              const dur   = 1 + (i * 0.041) % 0.8;
              const rep   = (i * 0.053) % 1.2;
              const rise  = 40 + (i * 17) % 60;
              const color = i % 3 === 0 ? "#FFD700" : i % 3 === 1 ? "#FFF8B0" : "#FFAA00";
              return (
                <motion.div
                  key={`p${i}`}
                  className="absolute rounded-full pointer-events-none"
                  style={{ width: size, height: size, background: color, left: `${left}%`, top: `${top}%` }}
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{ opacity: [0, 1, 0], scale: [0, 1.4, 0], y: [0, -rise] }}
                  transition={{ delay, duration: dur, repeat: Infinity, repeatDelay: rep }}
                />
              );
            })}
            ...
            <div className="relative z-10 flex items-center justify-center gap-5 px-6 w-full max-w-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <motion.p
                  initial={{ opacity: 0, x: -22 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                  style={{ fontSize: "1.05rem", fontWeight: 900, color: "#FFD700", textShadow: "0 0 18px #FFD700, 0 0 36px #FFD70066", lineHeight: 1.5 }}
                >{cinematic.line1}</motion.p>

                <motion.p
                  initial={{ opacity: 0, x: -22 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.75 }}
                  style={{ fontSize: "1.05rem", fontWeight: 900, color: "#FFD700", textShadow: "0 0 18px #FFD700, 0 0 36px #FFD70066", lineHeight: 1.5 }}
                >{cinematic.line2}</motion.p>

                <motion.p
                  initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.3 }}
                  className="text-yellow-200/45 text-[11px] font-semibold mt-3"
                >— {cinematic.name}</motion.p>
              ...
              <div className="flex-shrink-0 relative" style={{ width: 136 }}>
                ...
                >
                  ...
                  <motion.div
                    initial={{ opacity: 1 }} animate={{ opacity: 0 }} transition={{ delay: 0.25, duration: 0.4 }}
                    className="absolute inset-0 bg-white rounded-xl pointer-events-none"
                  />
                  <motion.div
                    initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.85, type: "spring", bounce: 0.6 }}
